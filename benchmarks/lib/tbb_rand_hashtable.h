...
public:
   ...
   V
   Get(const char *key) const; ///< Returns NULL if the key is not found
   bool
   Insert(const char *key, V value);
   V
   Update(const char *key, V value);
   V
   Remove(const char *key);
   std::vector<KVPair>
   Entries(const char *key = NULL, std::size_t n = -1) const;
   std::size_t
   Size() const
   {
      return table_.size();
...
private:
   struct HashEqual {
      uint64_t
      hash(const String &hstr) const
      {
         return hstr.hash();
      }
      bool
      equal(const String &a, const String &b) const
      {
         return a == b;
      }
   };
...
template<class V>
V
TbbRandHashtable<V>::Get(const char *key) const
{
...
template<class V>
bool
TbbRandHashtable<V>::Insert(const char *key, V value)
{
   if (!key)
      return false;
...
template<class V>
V
TbbRandHashtable<V>::Update(const char *key, V value)
{
   typename Hashtable::accessor       result;
   V                                  old(NULL);
   tbb::queuing_rw_mutex::scoped_lock lock(mutex_, false);
...
template<class V>
V
TbbRandHashtable<V>::Remove(const char *key)
{
   typename Hashtable::accessor       result;
   V                                  old(NULL);
   tbb::queuing_rw_mutex::scoped_lock lock(mutex_, false);
...
template<class V>
std::vector<typename TbbRandHashtable<V>::KVPair>
TbbRandHashtable<V>::Entries(const char *key, std::size_t n) const
{
