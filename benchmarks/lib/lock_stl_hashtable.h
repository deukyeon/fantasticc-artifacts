...
public:
   ...
   std::vector<KVPair>
   Entries(const char *key = NULL, size_t n = -1) const;
   std::size_t
   Size() const;
...
template<class V>
inline V
LockStlHashtable<V>::Get(const char *key) const
{
...
template<class V>
inline bool
LockStlHashtable<V>::Insert(const char *key, V value)
{
   std::lock_guard<std::mutex> lock(mutex_);
...
template<class V>
inline V
LockStlHashtable<V>::Update(const char *key, V value)
{
   std::lock_guard<std::mutex> lock(mutex_);
...
template<class V>
inline V
LockStlHashtable<V>::Remove(const char *key)
{
   std::lock_guard<std::mutex> lock(mutex_);
