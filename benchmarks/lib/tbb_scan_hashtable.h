...
private:
   struct Hash {
      uint64_t
      operator()(const String &hstr) const
      {
         return hstr.hash();
      }
   };
...
template<class V>
V
TbbScanHashtable<V>::Get(const char *key) const
{
...
template<class V>
bool
TbbScanHashtable<V>::Insert(const char *key, V value)
{
   if (!key)
      return false;
...
template<class V>
V
TbbScanHashtable<V>::Update(const char *key, V value)
{
   V                                  old(NULL);
...
template<class V>
V
TbbScanHashtable<V>::Remove(const char *key)
{
   V                                  old(NULL);
   tbb::queuing_rw_mutex::scoped_lock lock(mutex_);
   typename Hashtable::iterator       it = table_.find(String::Wrap(key));
...
template<class V>
std::vector<typename TbbScanHashtable<V>::KVPair>
TbbScanHashtable<V>::Entries(const char *key, std::size_t n) const
{
