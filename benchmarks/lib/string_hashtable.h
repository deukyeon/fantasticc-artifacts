...
public:
   ...
   virtual V
   Get(const char *key) const = 0; ///< Returns NULL if not found
   virtual bool
   Insert(const char *key, V value) = 0;
   virtual V
   Update(const char *key, V value) = 0;
   virtual V
   Remove(const char *key) = 0;
   virtual std::vector<KVPair>
   Entries(const char *key = NULL, std::size_t n = -1) const = 0;
   virtual std::size_t
   Size() const = 0;
