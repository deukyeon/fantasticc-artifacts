...
private:
   void
   HandleError(redisReply *reply, const char *hint);
   ...
   : slaves_(slaves)
{
   ...
   if (!context_ || context_->err) {
      if (context_) {
         ...
         std::cerr << "Connect error: can't allocate redis context!"
                   << std::endl;
      }
      exit(1);
   }
...
inline int
RedisClient::Command(std::string cmd)
{
   redisReply *reply;
...
inline void
RedisClient::HandleError(redisReply *reply, const char *hint)
{
