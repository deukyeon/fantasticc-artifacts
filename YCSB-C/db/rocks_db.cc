...
void
RocksDB::InitializeOptions(utils::Properties &props)
{
...
int
RocksDB::Read(const string         &table,
              const string         &key,
...
int
RocksDB::Scan(const string           &table,
              const string           &key,
              int                     len,
              const vector<string>   *fields,
...
int
RocksDB::Update(const string &table, const string &key, vector<KVPair> &values)
{
   return Insert(table, key, values);
}

int
RocksDB::Insert(const string &table, const string &key, vector<KVPair> &values)
{
   assert(values.size() == 1);
   rocksdb::Status status =
      db->Put(woptions, rocksdb::Slice(key), rocksdb::Slice(values[0].second));
   assert(status.ok());
   return DB::kOK;
}

int
RocksDB::Delete(const string &table, const string &key)
{
