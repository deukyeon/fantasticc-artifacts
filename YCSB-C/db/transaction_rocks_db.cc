...
void
TransactionRocksDB::InitializeOptions(utils::Properties &props)
{
   ...
   if (m.count("rocksdb.config_file")) {
      ...
      assert(LoadOptionsFromFile(
                copts, m.at("rocksdb.config_file"), &options, &cf_descs)
             == rocksdb::Status::OK());
   }
   ...
   for (auto tuple : m) {
      if (tuple.first.find("rocksdb.options.") == 0) {
         ...
         std::cout << "Unknown rocksdb config option " << tuple.first
                   << std::endl;
         assert(0);
      }
   }
...
TransactionRocksDB::TransactionRocksDB(utils::Properties &props, bool preloaded)
{
   ...
   rocksdb::Status status    = rocksdb::TransactionDB::Open(
      options, txndb_options, database_filename, &db);
   assert(status.ok());
   assert(isol_level != ROCKSDB_ISOLATION_LEVEL_INVALID);
}

TransactionRocksDB::~TransactionRocksDB()
{
   delete db;
}

void
TransactionRocksDB::Init()
{}

void
TransactionRocksDB::Close()
{}

void
TransactionRocksDB::Begin(Transaction **txn)
{
   if (*txn == NULL) {
...
int
TransactionRocksDB::Commit(Transaction **txn)
{
...
int
TransactionRocksDB::Read(Transaction                    *txn,
...
int
TransactionRocksDB::Scan(Transaction                      *txn,
                         const std::string                &table,
                         const std::string                &key,
                         int                               len,
                         const std::vector<std::string>   *fields,
...
int
TransactionRocksDB::Update(Transaction         *txn,
                           const std::string   &table,
                           const std::string   &key,
...
int
TransactionRocksDB::Insert(Transaction         *txn,
                           const std::string   &table,
                           const std::string   &key,
                           std::vector<KVPair> &values)
{
...
int
TransactionRocksDB::Delete(Transaction       *txn,
                           const std::string &table,
                           const std::string &key)
{
   assert(txn != NULL);
